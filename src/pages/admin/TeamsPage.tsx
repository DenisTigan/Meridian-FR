...
import type { DepartmentDto } from './DepartmentsPage';
...
export default function TeamsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [teamsRes, deptRes, empRes] = await Promise.all([
        apiClient.get<TeamDto[]>('/teams'), // In a real app we might pass departmentId here
  ...
  const handleOpenModal = (team?: TeamDto) => {
    if (team) {
      ...
      setEditingId(null);
      setFormData({ name: '', description: '', departmentId: filterDept, teamLeadId: '' });
    }
    setIsModalOpen(true);
  };
  ...
  const filteredTeams = filterDept ? teams.filter(t => t.departmentId === filterDept) : teams;
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Echipe</h1>
          <p className="text-slate-600">Administrare echipe</p>
        </div>
        <Button onClick={() => handleOpenModal()}>Adaugă Echipă</Button>
      </div>

      <div className="flex items-center gap-3 bg-slate-50 p-4 rounded-lg border border-slate-200">
        <label className="text-sm font-medium text-slate-700">Filtru Departament:</label>
        <select 
          ...
          value={filterDept}
          onChange={e => setFilterDept(e.target.value ? parseInt(e.target.value) : '')}
        >
        ...
        <table className="w-full text-sm text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              <tr><td colSpan={4} className="px-6 py-4 text-center text-slate-500">Nu există echipe.</td></tr>
            ) : (
              filteredTeams.map((team) => (
                <tr key={team.id} className="hover:bg-slate-50/50">
                  <td className="px-6 py-4 font-medium text-slate-900">{team.name}</td>
                  <td className="px-6 py-4 text-slate-600">{getDeptName(team.departmentId)}</td>
                  <td className="px-6 py-4 text-slate-600">{getEmployeeName(team.teamLeadId)}</td>
                  <td className="px-6 py-4 text-right space-x-2">
                    <Button variant="outline" size="sm" onClick={() => handleOpenModal(team)}>Editează</Button>
                    <Button variant="destructive" size="sm" onClick={() => handleDelete(team.id)}>Șterge</Button>
                  </td>
                </tr>
              ))
            )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center">
              <h2 className="text-xl font-semibold">{editingId ? 'Editează Echipă' : 'Adaugă Echipă'}</h2>
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <select 
                    ...
                    onChange={e => setFormData({...formData, teamLeadId: e.target.value ? parseInt(e.target.value) : ''})}
                  >
